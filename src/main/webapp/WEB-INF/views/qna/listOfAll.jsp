		...
		<table class="table">
			...
			<tbody>
				<c:forEach items="${pt.list}" var="vo" varStatus="i">
					<tr>
						<td>${i.count}</td>
						<td><div data-item_no="${vo.item_no}"
								data-item_name="${vo.item_name}"
								data-file_name="${vo.file_name}" class="uploadedList${i.index}"></div></td>
						<td><a style="text-decoration: none; color: #000;"
							href="/item/read/${vo.item_no}">${vo.item_name}</a></td>
						<td>${vo.qna_title}</td>
						<td>${vo.qna_regdate}</td>
						<td><a class="btn btn-outline-primary btn-sm"
							href="/qna/update/${vo.qna_no}">수정</a> <a
							class="btn btn-outline-primary btn-sm" class="delete"
							href="/qna/delete/${vo.qna_no}">삭제</a></td>
					</tr>
				</c:forEach>
			</tbody>
		</table>
