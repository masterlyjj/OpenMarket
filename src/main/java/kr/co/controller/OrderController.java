...
public class OrderController {
	...
	public String insertui(@PathVariable("member_id") String member_id, @PathVariable("item_no") int item_no,
			Model model) {
	...
	public String insert(MemberVO vo, String ilist, String receiver, @PathVariable("item_no") int item_no)
			throws Exception {
		...
		for (ItemVO m : itemlist) {
			orderList.add(
					new OrdersVO(0, m.getItem_no(), vo.getMember_id(), 1, m.getItem_price(), vo.getMember_address(),
							vo.getMember_detail_address(), vo.getMember_phone_number(), "상품 준비 중", receiver, null));
		}
		oService.insert(orderList);
	...
	public String detail(PageTO<OrdersVO> pt, @PathVariable("curPage") int curPage,
			@PathVariable("member_id") String member_id, Model model) {
